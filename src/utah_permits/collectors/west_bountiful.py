...
POLICY_ONLY_SIGNALS = (
    ...
    "model home update",
    "planning commission provisions",
    "home occupation",
    "street side yard",
)
...
class WestBountifulCollector:
    name = "West Bountiful"
    SCOPE_ID = "pmn-planning-agendas-v1"
    public_body_url = "https://www.utah.gov/pmn/sitemap/publicbody/1554.html"
    ...
    def collect(self, session: requests.Session | None = None) -> CollectionResult:
        ...
        if not permits:
            ...
            raise RuntimeError(f"West Bountiful planning source returned no usable records: {detail}")

        newest = max(p.issued_date for p in permits)
        note = (
            "Official Utah Public Notice West Bountiful Planning Commission agendas; "
            f"{len(permits)} project-specific planning/development item(s) from {pages_read} recent notice page(s), "
            f"latest substantive project meeting {newest}; conditional-use reviews, plats, subdivisions, site plans "
            "and site-specific zoning/development actions retained; code-policy, ADU, home-occupation and "
    ...
    def parse_notice_page(cls, html: str, source_url: str) -> list[Permit]:
        ...
        lowered = text.lower()
        if "west bountiful" not in lowered or "planning commission" not in lowered:
            return []
        if "planning commission meeting - canceled" in lowered or "planning commission meeting - cancelled" in lowered:
            return []
        ...
        for item in cls._numbered_items(description):
            ...
            permits.append(
                Permit(
                    ...
                    source_name="Utah Public Notice - West Bountiful Planning Commission",
    ...
    def _project_name(cls, item: str, address: str, permit_type: str) -> str:
        patterns = (
            ...
            r"Site Plan(?: Review)?\s*-?\s*(.+?)(?:\s*-\s*\d|$)",
            r"(?:Zone Change|Rezone|Zoning Map Amendment)\s*-?\s*(.+?)(?:\s*-\s*\d|$)",
        )
        ...
        return f"West Bountiful {label}"[:180]
