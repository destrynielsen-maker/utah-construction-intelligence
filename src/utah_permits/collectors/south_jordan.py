from __future__ import annotations

import re
from datetime import datetime
from urllib.parse import urljoin

import requests
from bs4 import BeautifulSoup

from .base import CollectionResult, new_session
from ..models import Permit


POLICY_ONLY_SIGNALS = (
    "accessory dwelling",
    "adu",
    "city code",
    "legislative updates",
    "ordinance no.",
)


class SouthJordanCollector:
    name = "South Jordan"
    SCOPE_ID = "pmn-planning-public-hearings-v1"
    public_body_url = "https://www.utah.gov/pmn/sitemap/publicbody/1032.html"
    notices_url = public_body_url
    MAX_NOTICE_PAGES = 8

    def collect(self, session: requests.Session | None = None) -> CollectionResult:
        session = session or new_session()
        response = session.get(self.public_body_url, timeout=45)
        response.raise_for_status()
        notice_urls = self.discover_notice_urls(response.text, response.url)

        permits: list[Permit] = []
        errors: list[str] = []
        for notice_url in notice_urls[: self.MAX_NOTICE_PAGES]:
            try:
                notice = session.get(notice_url, timeout=45)
                notice.raise_for_status()
                permits.extend(self.parse_notice_page(notice.text, notice.url))
            except Exception as exc:
                errors.append(f"{type(exc).__name__}: {exc}")

        by_application: dict[str, Permit] = {}
        for permit in permits:
            old = by_application.get(permit.permit_number)
            if old is None or permit.issued_date > old.issued_date:
                by_application[permit.permit_number] = permit

        permits = sorted(
            by_application.values(),
            key=lambda p: (p.issued_date, p.project_name or ""),
            reverse=True,
        )
        if not permits:
            detail = "; ".join(errors) if errors else "no project-specific public-hearing items parsed"
            raise RuntimeError(f"South Jordan planning source returned no usable records: {detail}")

        newest = max(p.issued_date for p in permits)
        note = (
            "Official Utah Public Notice South Jordan Planning Commission public-hearing notices; "
            f"{len(permits)} project-specific planning/development item(s), latest hearing {newest}; "
            "application numbers and addresses retained when available; ADU/code-policy items excluded; "
            "planning/development-stage intelligence only"
        )
        if errors:
            note += f"; {len(errors)} notice page(s) unavailable"

        return CollectionResult(
            self.name,
            permits,
            self.public_body_url,
            note,
            scope_id=self.SCOPE_ID,
        )

    @classmethod
    def discover_notice_urls(cls, html: str, source_url: str) -> list[str]:
        soup = BeautifulSoup(html, "html.parser")
        found: list[str] = []
        seen: set[str] = set()
        for anchor in soup.find_all("a", href=True):
            href = anchor.get("href", "")
            if "/pmn/sitemap/notice/" not in href:
                continue
            text = cls._clean(" ".join(anchor.stripped_strings)).lower()
            if "notice of public hearing" not in text:
                continue
            if "cancel" in text:
                continue
            url = urljoin(source_url, href)
            if url not in seen:
                seen.add(url)
                found.append(url)
        return found

    @classmethod
    def parse_notice_page(cls, html: str, source_url: str) -> list[Permit]:
        soup = BeautifulSoup(html, "html.parser")
        text = cls._clean(soup.get_text(" ", strip=True))
        lowered = text.lower()
        if "cancelled" in lowered or "canceled" in lowered:
            return []

        event_date = cls._event_date(text)
        description = cls._description(text)
        if not event_date or not description:
            return []

        permits: list[Permit] = []
        for application_number, item in cls._application_items(description):
            normalized = item.lower()
            if any(signal in normalized for signal in POLICY_ONLY_SIGNALS):
                continue
            if application_number.startswith("PLZTA"):
                continue

            permits.append(
                Permit(
                    state="UT",
                    jurisdiction=cls.name,
                    permit_number=f"SOJ-PLAN-{application_number}",
                    issued_date=event_date,
                    permit_type=cls._permit_type(application_number, item),
                    address=cls._address(item),
                    project_name=cls._project_name(item),
                    units=cls._units(item),
                    status="Planning Commission Public Hearing",
                    source_name="Utah Public Notice - South Jordan Planning Commission",
                    source_url=source_url,
                    raw={
                        "lead_stage": "PLANNING",
                        "hearing_date": event_date,
                        "date_semantics": "scheduled_public_hearing_date",
                        "application_number": application_number,
                        "hearing_item": item,
                    },
                )
            )
        return permits

    @staticmethod
    def _clean(value: str) -> str:
        return re.sub(r"\s+", " ", value or "").strip(" .")

    @staticmethod
    def _event_date(text: str) -> str | None:
        match = re.search(
            r"Event Start Date & Time\s+([A-Za-z]+\s+\d{1,2},\s+\d{4})",
            text,
            flags=re.I,
        )
        if not match:
            return None
        try:
            return datetime.strptime(match.group(1), "%B %d, %Y").date().isoformat()
        except ValueError:
            return None

    @classmethod
    def _description(cls, text: str) -> str:
        match = re.search(
            r"Description/Agenda\s+(.*?)(?:Notice of Special Accommodations|Meeting Information|Notice Posting Details|Download Attachments|$)",
            text,
            flags=re.I,
        )
        return cls._clean(match.group(1)) if match else ""

    @classmethod
    def _application_items(cls, description: str) -> list[tuple[str, str]]:
        body = re.split(r"\s+The meeting may also be joined\b", description, maxsplit=1, flags=re.I)[0]
        matches = re.finditer(
            r"(?:^|\s+-\s+)(PL[A-Z]{2,6}\d{6,})\s+-\s+(.*?)(?=\s+-\s+PL[A-Z]{2,6}\d{6,}\s+-\s+|$)",
            body,
            flags=re.I,
        )
        return [(m.group(1).upper(), cls._clean(m.group(2))) for m in matches]

    @classmethod
    def _project_name(cls, item: str) -> str:
        first = item.split(",", 1)[0]
        return cls._clean(first)

    @staticmethod
    def _permit_type(application_number: str, item: str) -> str:
        lowered = item.lower()
        if application_number.startswith("PLSPR") or "site plan" in lowered:
            return "Planning Site Plan"
        if application_number.startswith("PLPP") or "preliminary subdivision" in lowered:
            return "Planning Subdivision/Plat"
        if application_number.startswith("PLZBA") or "rezone" in lowered or "zoning amendment" in lowered:
            return "Planning Rezone"
        if application_number.startswith("PLCUP") or "conditional use permit" in lowered or "cup" in lowered:
            return "Planning Conditional Use"
        if application_number.startswith("PLMPA") or "major site plan amendment" in lowered:
            return "Planning Site Plan Amendment"
        return "Planning Review"

    @classmethod
    def _address(cls, item: str) -> str:
        for part in item.split(","):
            candidate = cls._clean(part)
            if re.match(r"^\d{2,5}\s+", candidate):
                return candidate
        return ""

    @staticmethod
    def _units(item: str) -> int | None:
        for pattern in (
            r"\b(\d{1,4})-lot\b",
            r"\b(\d{1,4})\s+(?:residential\s+)?lots\b",
            r"\b(\d{1,4})\s+units\b",
        ):
            match = re.search(pattern, item, flags=re.I)
            if match:
                return int(match.group(1))
        return None
