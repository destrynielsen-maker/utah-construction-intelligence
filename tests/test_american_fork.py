...
class AmericanForkTests(unittest.TestCase):
    ...
    def test_public_notices_keep_development_hearings_only(self) -> None:
        html = """
        <html><body>
        <p>09.14.2026 Public Hearing - North Pointe Business Park, Building C - Amended Final Plat</p>
        <p>08.19.2026 Public Hearing - Chipman and Stake Center 2 Annexation</p>
        <p>09.02.2026 Public Hearing - Chapter 12.16 Park Strips and Trees - Code Text Amendment</p>
        </body></html>
        """
        permits = AmericanForkCollector.parse_notices_html(html, "https://example.test/notices")

        self.assertEqual(2, len(permits))
        self.assertEqual("2026-09-14", permits[0].issued_date)
